var _carbonads = {
    init: function() {
        ...
        _carbon_dev = (this.getUrlVar('zoneid') == 1386 || this.getUrlVar('serve') == 'C6SI42Y');
        _carbon_design = (this.getUrlVar('zoneid') == 1408 || this.getUrlVar('serve') == 'C6SIVKV');
        _carbon_ux = (this.getUrlVar('zoneid') == 1409 || this.getUrlVar('serve') == 'C6SIVKE');
        _carbon_serve = this.getServe(this.getUrlVar('serve'), _carbon_placement);
    ...
    reroute: function(slug) {
        var p = new Array();
        p['markitupjaysalvatcom'] = 'F6AIK5T';
        p['wwwadrianpelletiercom'] = 'F6AIK5T';
        p['wwwbuildinternetcom'] = 'F6AIK5T';
        p['blogechoenduringcom'] = 'F6AIK5T';
        p['978gs'] = 'F6AIK5T';
        p['stierncom'] = 'F6AIK5T';
        p['aaronrussellcouk'] = 'F6AIK5T';
        p['tympanusnet'] = 'F6AIK5T';
        p['blogdavidtateorg'] = 'F6AIK5T';
        p['wwwsqlreplcom'] = 'F6AIK5T';
        p['wwwravelrumbacom'] = 'F6AIK5T';
        p['tnycz'] = 'F6AIK5T';
        p['bootswatchcom'] = 'F6AIK5T';
        p['johnpolacekcom'] = 'F6AIK5T';
        p['lovebootstrapcom'] = 'F6AIK5T';
        p['geocoderca'] = 'F6AIK5T';
        p['drublicde'] = 'F6AIK5T';
        p['alphapixelscom'] = 'F6AIK5T';
        p['designerschoolcom'] = 'F6AIK5T';
        p['blogtipsntrickscom'] = 'F6AIK5T';
        p['blogharrywolffcom'] = 'F6AIK5T';
        p['dakerme'] = 'F6AIK5T';
        p['mattsnidercom'] = 'F6AIK5T';
        p['haydenjamesio'] = 'F6AIK5T';
        p['responsiveimgcom'] = 'F6AIK5T';
        p['charactercodecom'] = 'F6AIK5T';
        p['scottohara'] = 'F6AIK5T';
        p['breachcc'] = 'F6AIK5T';
        p['pankajparashar'] = 'F6AIK5T';
        p['semanticgs'] = 'F6AIK5T';
        p['pastebinadpacks'] = 'F6AIK5T';
        p['codepen'] = 'F6AIK5T';
        p['gistthemes'] = 'F6AIK5T';
        p['ezdz'] = 'F6AIK5T';
        p['sveg'] = 'F6AIK5T';
        p['markitupjaysalvat'] = 'F6AIK5T';
        p['buzzjaysalvat'] = 'F6AIK5T';
        p['vegasjaysalvat'] = 'F6AIK5T';
        p['facedetectionjaysalvat'] = 'F6AIK5T';
        p['wwwstudentguidewebdesigncom'] = 'F6AIK5T';
        p['addorncom'] = 'F6AIK5T';
        p['ninetofiveme'] = 'F6AIK5T';
        p['layerform'] = 'F6AIK5T';
        p['sideprojectsin'] = 'F6AIK5T';
        p['wwwdiscussingdesigncom'] = 'F6AIK5T';
        p['shortboredsurfercom'] = 'F6AIK5T';
        p['wwwsimonpancom'] = 'F6AIK5T';
        p['wwwthisisaaronslifecom'] = 'F6AIK5T';
        p['uxadaycom'] = 'F6AIK5T';
        p['wwwgetfinchcom'] = 'F6AIK5T';
        p['wwwfontfacecom'] = 'C6AILKT';
        p['5thirtyonecom'] = 'CVYD42E';
        p['osvaldasinfo'] = 'C6AILKT';
        p['cultttcom'] = 'C6AILKT';
        p['freshas'] = 'C6AILKT';
        p['uxmasterycom'] = 'CVYD42E';
        p['wwwelezeacom'] = 'CVYD42E';
        p['wwwhelloerikcom'] = 'CVYD42E';
        p['uipatternscom'] = 'CVYD42E';
        p['wwwjankoatwarpspeedcom'] = 'C6AILKT';
        p['getwirefycom'] = 'C6AILKT';
        if (this.isset(p[slug]))
            return p[slug];
        else
            return null
    },
    getServe: function(s, p) {
        if (this.isset(this.reroute(p)))
            return this.reroute(p);
        if (_carbon_dev)
            return 'C6AILKT';
        if (_carbon_design)
            return 'CVYD42T';
    ...
    getUrlVar: function(name) {
        ...
        var results = regex.exec(document.getElementById('_adpacks_js').src);
        if (results == null)
            return '';
        else
            return results[1]
    ...
    reload: function() {
        ...
        this.remove(document.getElementById('_legacy_carbonads_css'));
        this.remove(document.getElementById('adpacks'));
        this.init()
    },
    remove: function(el) {
...
function _carbonads_go(b) {
    ...
    if (_carbonads.isNum(_carbon_legacyid)) {
        ...
        el.className = 'bsap';
        el.innerHTML = '<div class="bsa_it one"><div class="bsa_it_ad"><a href="' + fulllink.replace('[timestamp]', time) + '" target="_blank"><span class="bsa_it_i"><img src="' + ad.image + '" width="130" height="100" alt=""></span></a><a href="' + fulllink.replace('[timestamp]', time) + '" target="_blank"><span class="bsa_it_d">' + ad.description + '</span></a><div style="clear:both"></div></div><span class="bsa_it_p"><a href="http://carbonads.net/" target="_blank">ads via Carbon</a></span></div>'
    } else {
        var el = document.createElement('span');
        el.id = 'adpacks';
        el.innerHTML = '<span class="adpacks-wrap"><a href="' + fulllink.replace('[timestamp]', time) + '" class="adpacks-img" target="_blank"><img src="' + ad.image + '" alt="" border="0" height="100" width="130" style="max-width:130px;" /></a><a href="' + fulllink.replace('[timestamp]', time) + '" class="adpacks-text" target="_blank">' + ad.description + '</a></span><a href="http://carbonads.net/" target="_blank" class="adpacks-poweredby">ads via Carbon</a>'
    }
    ...
    if (carbonjs != null)
        carbonjs.parentNode.insertBefore(el, carbonjs.nextSibling);
